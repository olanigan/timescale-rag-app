...
Performance dashboard and usage statistics for the RAG system.
Shows metrics, charts, and system performance data.
"""
...
st.set_page_config(
    page_title="Analytics - RAG Showcase",
    page_icon="📈",
    layout="wide"
)
...
def main():
    ...
    st.markdown("Performance metrics and usage statistics for the RAG system")
    ...
    with tab1:
        show_overview_dashboard(vec_store)

    with tab2:
        show_performance_metrics(vec_store)

    with tab3:
        show_search_analytics(vec_store)

    with tab4:
        show_system_health(vec_store)

def show_overview_dashboard(vec_store):
...
def show_performance_metrics(vec_store):
...
def show_search_analytics(vec_store):
    """Show search analytics and patterns"""
...
def show_system_health(vec_store):
...
def run_performance_test(vec_store):
